...
type View='dashboard'|'new'|'edit'|'cadastros'|'usuarios'|'reports'|'administration'|'archived';
...
export default function App(){
 ...
 return <div className="institution-shell">
   <header className="municipal-header"><div className="department-title"><Building2 size={18}/><div><b>Departamento de Engenharia</b><span>S.O.S — Sistema de Ordens de Manutenção</span></div></div><div className="user-chip"><strong>{session.name}</strong><span>{session.role}</span></div></header>
 ...
 {isAdmin&&<button className={view==='administration'?'active':''} onClick={()=>{setSelected(null);setView('administration')}}><Settings size={18}/>Administração</button>}
 </nav><button className="logout" onClick={signout}><LogOut size={18}/>Sair</button></aside><main className="main">{view==='cadastros'?<Cadastros catalogs={catalogs} onChange={setCatalogs} isAdmin={isAdmin}/>:view==='usuarios'&&isAdmin?<Usuarios users={users} onChange={setUsers}/>:view==='reports'?<Reports orders={orders} onOpen={id=>{setSelected(id);setView('dashboard')}}/>:view==='archived'?<ArchivedOrders orders={orders} onOpen={id=>{setSelected(id);setView('dashboard')}}/>:view==='administration'&&isAdmin?<Administration orders={orders} users={users} catalogs={catalogs}/>:view==='new'?<WorkOrderForm catalogs={catalogs} number={nextOrderNumber(orders)} onCancel={goDashboard} onSave={save}/>:view==='edit'&&current?<WorkOrderForm catalogs={catalogs} initial={current} number={current.number} onCancel={()=>setView('dashboard')} onSave={save}/>:current?<WorkOrderDetail os={current} onBack={goDashboard} onEdit={()=>setView('edit')} onChange={save} onDelete={()=>remove(current.id)}/>:<Dashboard orders={orders} onOpen={id=>{setSelected(id);setView('dashboard')}} onNew={()=>setView('new')}/>}</main></div>
