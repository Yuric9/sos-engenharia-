...
export function saveUsers(v:AppUser[]):boolean{try{localStorage.setItem(USERS,JSON.stringify(v));return true}catch(error){console.error('Não foi possível salvar os usuários no armazenamento local.',error);return false}}
