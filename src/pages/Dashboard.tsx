...
type DashboardFilter='TODAS'|'ABERTA'|'EM_ANDAMENTO'|'PARALISADAS'|'ATENDIDA'|'CONCLUIDA'|'ATRASADAS'|'ARQUIVADAS';

export default function Dashboard({orders,onOpen,onNew,onBulkStatus}:{orders:WorkOrder[];onOpen:(id:number)=>void;onNew:()=>void;onBulkStatus:(ids:number[],status:StatusOS)=>Promise<boolean>}){
 ...
 const cards:[string,number,any,DashboardFilter][]=[
  ...
  ['Atendidas',monthScopedOperational.filter(x=>x.status==='ATENDIDA').length,CheckCircle2,'ATENDIDA'],
   ['Concluídas',monthScopedOperational.filter(x=>x.status==='CONCLUIDA').length,CheckCircle2,'CONCLUIDA'],
 ...
 const changeStatus=async(os:WorkOrder,status:StatusOS)=>{if(status===os.status){setEditingStatusId(null);return}setSavingStatusId(os.id);try{const ok=await onBulkStatus([os.id],status);if(ok)setEditingStatusId(null)}finally{setSavingStatusId(null)}};
 ...
 <div className="filters">{([['TODAS','Todas ativas'],['ABERTA','Abertas'],['EM_ANDAMENTO','Em andamento'],['PARALISADAS','Paralisadas'],['ATENDIDA','Atendidas'],['CONCLUIDA','Concluídas'],['ATRASADAS','Atrasadas'],['ARQUIVADAS','Arquivadas']] as [DashboardFilter,string][]).map(([v,l])=><button key={v} className={filter===v?'selected':''} onClick={()=>setFilter(v)}>{l}</button>)}{monthFilter!==null&&<button className="selected" onClick={()=>setMonthFilter(null)}>Limpar mês: {monthLabels[monthFilter]}</button>}</div>
 <div className="table-scroll"><table><thead><tr><th>O.S.</th><th>Data</th><th>Origem</th><th>Secretaria / Unidade</th><th>Serviço</th><th>Prioridade</th><th>Equipe</th><th>Prazo / Tempo</th><th>Situação</th><th></th></tr></thead><tbody>{visible.map(os=> <tr key={os.id} onClick={e=>{if((e.target as HTMLElement).closest('button,select'))return;onOpen(os.id)}}><td><b>#{os.number}/{yearOf(os)}</b>{os.officeDocument&&<span className="sub"><Paperclip size={12}/>{os.officeDocument}</span>}</td><td>{new Date(os.openedAt+'T12:00:00').toLocaleDateString('pt-BR')}</td><td>{os.importOrigin||'Cadastro manual'}</td><td><b>{os.secretaria}</b><span className="sub">{os.unidade}</span></td><td><b>{os.serviceType}</b><span className="sub clamp">{os.description}</span></td><td><span style={priorityStyle(os.priority)}>{os.priority}</span></td><td>{os.team}</td><td><b>{new Date(os.deadline+'T12:00:00').toLocaleDateString('pt-BR')}</b> <span className="sub">{os.estimatedAmount} {os.estimatedUnit==='DIARIAS'?(os.estimatedAmount===1?'diária':'diárias'):'horas'}</span></td><td onClick={e=>e.stopPropagation()}>{editingStatusId===os.id?<select autoFocus value={os.status} disabled={savingStatusId===os.id} onChange={e=>changeStatus(os,e.target.value as StatusOS)} onBlur={()=>setEditingStatusId(null)} aria-label={`Alterar status da O.S. ${os.number}`}><option value="ABERTA">Aberta</option><option value="EM_ANDAMENTO">Em andamento</option><option value="PARALISADA">Paralisada</option><option value="AGUARDANDO_MATERIAL">Aguardando material</option> <option value="ATENDIDA">Atendida</option><option value="CONCLUIDA">Concluída</option><option value="CANCELADA">Cancelada</option></select>:<button type="button" className={`status s-${os.status.toLowerCase()}`} onClick={()=>setEditingStatusId(os.id)} title="Clique para alterar o status">{os.status.replaceAll('_',' ')}</button>}{os.archived&&<span className="sub">Arquivada</span>}{!os.archived&&os.overdueDays>0&&<span className="overdue">{os.overdueDays} dias atrasada</span>}</td><td><ChevronRight size={18}/></td></tr>)}{visible.length===0&&<tr><td colSpan={10} style={{textAlign:'center',padding:28}}>Nenhuma O.S. encontrada para os filtros selecionados.</td></tr>}</tbody></table></div></section></>
}
