...
export default function Dashboard({orders,onOpen,onNew}:{orders:WorkOrder[];onOpen:(id:number)=>void;onNew:()=>void}){
 ...
 const visible=useMemo(()=>orders.filter(o=>{
  const text=`${o.number} ${o.secretaria} ${o.unidade} ${o.serviceType} ${o.team}`.toLowerCase();
 ...
 const monthIndexes=orders.flatMap(o=>{if(!o.openedAt)return [];const d=new Date(o.openedAt+'T12:00:00');return Number.isFinite(d.getTime())&&d.getFullYear()===currentYear?[d.getMonth()]:[]});
 const lastMonthWithOrders=monthIndexes.length?Math.max(...monthIndexes):0;
 const months=Array.from({length:lastMonthWithOrders+1},(_,i)=>orders.filter(o=>{if(!o.openedAt)return false;const d=new Date(o.openedAt+'T12:00:00');return Number.isFinite(d.getTime())&&d.getFullYear()===currentYear&&d.getMonth()===i}).length);
 ...
 <section className="chart-card"><div><h2>O.S. por mês</h2><p>Quantidade cadastrada em {currentYear}, até o último mês com O.S. • clique no mês para filtrar</p></div><div className="bars">{months.map((v,i)=><button type="button" className={`bar-wrap${monthFilter===i?' selected':''}`} key={i} onClick={()=>setMonthFilter(monthFilter===i?null:i)} title={`Mostrar O.S. de ${monthLabels[i]} de ${currentYear}`} style={{background:'transparent',border:0,cursor:'pointer',padding:0}}><strong style={{fontSize:12}}>{v}</strong><div className="bar" style={{height:v===0?'0%':`${Math.max(8,(v/max)*100)}%`}}/><small>{monthLabels[i]}</small></button>)}</div></section>
 ...
 <div className="table-scroll"><table><thead><tr><th>O.S.</th><th>Data</th><th>Secretaria / Unidade</th><th>Serviço</th><th>Equipe</th><th>Prazo / Tempo</th><th>Situação</th><th></th></tr></thead><tbody>{visible.map(os=><tr key={os.id} onClick={()=>onOpen(os.id)}><td><b>#{os.number}</b>{os.officeDocument&&<span className="sub"><Paperclip size={12}/>{os.officeDocument}</span>}</td><td>{new Date(os.openedAt+'T12:00:00').toLocaleDateString('pt-BR')}</td><td><b>{os.secretaria}</b><span className="sub">{os.unidade}</span></td><td><b>{os.serviceType}</b><span className="sub clamp">{os.description}</span></td><td>{os.team}</td><td><b>{new Date(os.deadline+'T12:00:00').toLocaleDateString('pt-BR')}</b><span className="sub">{os.estimatedAmount} {os.estimatedUnit.toLowerCase()}</span></td><td><span className={`status s-${os.status.toLowerCase()}`}>{os.status.replaceAll('_',' ')}</span>{os.archived&&<span className="sub">Arquivada</span>}{os.overdueDays>0&&<span className="overdue">{os.overdueDays} dias atrasada</span>}</td><td><ChevronRight size={18}/></td></tr>)}</tbody></table></div></section></>
}
