...
export default function WorkOrderForm({initial,number,onCancel,onSave,catalogs}:{initial?:WorkOrder;number:number;onCancel:()=>void;onSave:(os:WorkOrder)=>void;catalogs:Catalogs}){
 ...
 const submit=(e:any)=>{e.preventDefault();if(!f.secretaria||!f.unidade||!f.serviceType||!f.description){alert('Preencha Secretaria, Unidade, Tipo de serviço e Descrição.');return;}onSave(f)};
 ...
 <label><span>Secretaria</span><select style={inputStyle} value={f.secretaria} onChange={e=>{setF(x=>({...x,secretaria:e.target.value,unidade:''}))}}><option value="">Selecione</option>{active('secretarias').map(x=><option key={x.id}>{x.name}</option>)}</select></label>
 <label><span>Unidade / Órgão</span><select style={inputStyle} value={f.unidade} onChange={e=>set('unidade',e.target.value)}><option value="">Selecione</option>{units.map(x=><option key={x.id}>{x.name}</option>)}</select></label>
 <label><span>Local / Setor</span><input style={inputStyle} value={f.local||''} onChange={e=>set('local',e.target.value)}/></label>
