...
export default function WorkOrderDetail({os,onBack,onEdit,onChange,onDelete,canDelete}:{os:WorkOrder;onBack:()=>void;onEdit:()=>void;onChange:(os:WorkOrder)=>void;onDelete:()=>void;canDelete:boolean}){
 ...
 const openAttachment=async(a:Attachment)=>{
  let url=a.dataUrl||previews[a.id]||null;
  ...
  const w=window.open();
  if(w){w.document.title=a.name;w.location.href=url}else alert('Não foi possível abrir uma nova janela para o arquivo.');
 };
 const delAttachment=async(a:Attachment)=>{
  if(!confirm(`Excluir o arquivo "${a.name}" desta O.S.?\n\nEssa ação remove o arquivo armazenado e não poderá ser desfeita.`))return;
 ...
 <article className="panel"><div className="panel-title"><h3>Materiais</h3><button onClick={onEdit}><Plus size={15}/>Editar</button></div><p>{os.materialsSummary||'Nenhum material informado.'}</p>{materialPdfs.map(a=><div className="mini-row" key={a.id}><FileText size={17}/><div><b>{a.name}</b><span>Lista de materiais em PDF</span></div><div style={{display:'flex',gap:6}}><button onClick={()=>openAttachment(a)}><ExternalLink size={14}/>Abrir</button><button onClick={()=>delAttachment(a)}><Trash2 size={14}/>Excluir</button></div></div>)}<div className="mini-row"><Package size={17}/><span>Registro de materiais vinculado à O.S.</span></div></article>
 ...
 <div className="attachment-summary">{(os.attachments||[]).length===0?<div><Camera/><strong>Nenhum anexo</strong><span>Adicione fotos, ofícios, prints ou comprovantes.</span></div>:(os.attachments||[]).map(a=><div key={a.id}>{a.type.startsWith('image/')&&(a.dataUrl||previews[a.id])?<button type="button" onClick={()=>openAttachment(a)} title="Abrir imagem" style={{border:0,padding:0,background:'transparent',width:'100%',cursor:'pointer'}}><img src={a.dataUrl||previews[a.id]} alt={a.name} style={{width:'100%',height:110,objectFit:'cover',borderRadius:8,display:'block'}}/></button>:<FileText size={36}/>}<strong title={a.name} style={{overflowWrap:'anywhere'}}>{a.name}</strong><span>{a.category}{a.sizeBytes?` • ${(a.sizeBytes/1024).toFixed(0)} KB`:''}</span><div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}><button type="button" onClick={()=>openAttachment(a)}><ExternalLink size={14}/>Abrir</button><button type="button" onClick={()=>delAttachment(a)}><Trash2 size={14}/>Excluir</button></div></div>)}</div><p className="hint">Clique em <b>Abrir</b> ou na própria foto para visualizar. A opção <b>Excluir</b> pede confirmação antes de remover o arquivo do armazenamento.</p><p className="hint">{desktop?'Os anexos desta versão são gravados fisicamente na pasta sos-data/anexos do HD externo.':'Na versão web, anexos pequenos permanecem no armazenamento local para compatibilidade.'}</p></article>
