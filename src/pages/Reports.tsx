...
import { BarChart3, Search, Printer, Clock3, MessageSquareText, X } from 'lucide-react';
...
export default function Reports({orders,onOpen}:{orders:WorkOrder[];onOpen:(id:number)=>void}){
  ...
  return <>
    ...
    <section className="table-card"><div className="table-toolbar"><div><h2>Filtros do relatório</h2><p>Selecione período, situação e pesquisa.</p></div></div><div className="filters" style={{alignItems:'center',gap:10,flexWrap:'wrap'}}><select value={year} onChange={e=>{setYear(Number(e.target.value));setDrill(null)}}>{years.map(y=><option key={y} value={y}>{y}</option>)}</select><select value={month} onChange={e=>{setMonth(Number(e.target.value));setDrill(null)}}>{monthLabels.map((m,i)=><option key={m} value={i}>{m}</option>)}</select><select value={status} onChange={e=>{setStatus(e.target.value);setDrill(null)}}><option value="TODOS">Todas as situações</option><option value="ABERTA">Abertas</option><option value="EM_ANDAMENTO">Em andamento</option><option value="PARALISADA">Paralisadas</option><option value="AGUARDANDO_MATERIAL">Aguardando material</option><option value="ATENDIDA">Atendidas</option><option value="CONCLUIDA">Concluídas</option><option value="CANCELADA">Canceladas</option><option value="ARQUIVADAS">Arquivadas</option></select><div className="search"><Search size={17}/><input value={q} onChange={e=>{setQ(e.target.value);setDrill(null)}} placeholder="Número, secretaria, unidade, serviço..."/></div></div></section>
