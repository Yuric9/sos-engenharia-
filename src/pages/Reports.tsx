...
const openStatuses:WorkOrder['status'][]=['ABERTA','EM_ANDAMENTO','PARALISADA','AGUARDANDO_MATERIAL'];
const statusLabels:Record<WorkOrder['status'],string>={ABERTA:'Aberta',EM_ANDAMENTO:'Em andamento',PARALISADA:'Paralisada',AGUARDANDO_MATERIAL:'Aguardando material',ATENDIDA:'Atendida',CONCLUIDA:'Concluída',CANCELADA:'Cancelada'};
...
export default function Reports({orders,onOpen,onBulkCharge}:{orders:WorkOrder[];onOpen:(id:number)=>void;onBulkCharge:(ids:number[],context:string)=>Promise<boolean>|boolean}){
  ...
  const [chargeTeam,setChargeTeam]=useState('TODAS');const [chargeStatus,setChargeStatus]=useState('TODAS_ABERTAS');const [chargeOpen,setChargeOpen]=useState(false);const [chargeDraft,setChargeDraft]=useState('');
  ...
  const chargePeriodOrders=useMemo(()=>orders.filter(o=>matchesPeriod(o)&&matchesSearch(o)&&!o.archived&&openStatuses.includes(o.status)),[orders,year,month,q]);
  ...
  const chargeCandidates=useMemo(()=>chargePeriodOrders.filter(o=>{
    const matchesChargeStatus=chargeStatus==='TODAS_ABERTAS'||(chargeStatus==='ATRASADAS'?o.overdueDays>0:o.status===chargeStatus);
    const matchesTeam=chargeTeam==='TODAS'||((o.team||'Não informado').trim()||'Não informado')===chargeTeam;
    return matchesChargeStatus&&matchesTeam;
  }).sort((a,b)=>b.overdueDays-a.overdueDays||a.number-b.number),[chargePeriodOrders,chargeTeam,chargeStatus]);
  const chargeContext=`${monthLong[month]} de ${year}${chargeTeam==='TODAS'?'':` • ${chargeTeam}`}`;
  const buildBulkCharge=()=>{
    const lines=chargeCandidates.map(o=>{
      const yearOpened=new Date(o.openedAt+'T12:00:00').getFullYear();
      const delay=o.overdueDays>0?` • ${o.overdueDays} ${o.overdueDays===1?'dia':'dias'} em atraso`:'';
      return `• O.S. ${o.number}/${yearOpened} | Data: ${fmt(o.openedAt)} | Serviço: ${o.serviceType}\n  Descrição: ${(o.description||'Não informada').trim()}\n  Local: ${o.unidade||o.secretaria}${o.local?` — ${o.local}`:''} | Prazo: ${fmt(o.deadline)} | Situação: ${statusLabels[o.status]}${delay}`;
    });
    const heading=chargeStatus==='ATRASADAS'?'que permanecem em atraso':chargeStatus==='TODAS_ABERTAS'?'que permanecem em aberto':`com situação "${statusLabels[chargeStatus as WorkOrder['status']]}"`;
    return `SOLICITAÇÃO DE ATUALIZAÇÃO DAS ORDENS DE SERVIÇO\n\nPrezados, solicitamos a atualização e a justificativa referente às Ordens de Serviço abaixo, ${heading}:\n\n${lines.join('\n\n')}\n\nPara cada O.S., favor informar claramente a situação atual, o que impede ou falta para a execução e a previsão de conclusão.\n\nDepartamento de Engenharia`;
  };
  const openBulkCharge=()=>{if(!chargeCandidates.length){alert('Não há O.S. abertas para os filtros selecionados.');return}setChargeDraft(buildBulkCharge());setChargeOpen(true)};
  ...
  return <>
    ...
    <section className="table-card" style={{marginBottom:12}}><div className="table-toolbar"><div><h2>Cobrança em lote</h2><p>Selecione todas as O.S. abertas, somente as atrasadas ou um status específico. O.S. arquivadas, atendidas, concluídas e canceladas nunca entram.</p></div><button className="primary" onClick={openBulkCharge} disabled={!chargeCandidates.length}><MessageSquareText size={17}/>Gerar cobrança ({chargeCandidates.length})</button></div><div className="filters" style={{alignItems:'center',gap:10,flexWrap:'wrap'}}><span><b>Período:</b> {monthLong[month]} de {year}</span><select value={chargeStatus} onChange={e=>{setChargeStatus(e.target.value);setChargeOpen(false)}}><option value="TODAS_ABERTAS">Todas as O.S. abertas</option><option value="ATRASADAS">Somente atrasadas</option>{openStatuses.map(value=><option key={value} value={value}>{statusLabels[value]}</option>)}</select><select value={chargeTeam} onChange={e=>{setChargeTeam(e.target.value);setChargeOpen(false)}}><option value="TODAS">Todas as equipes / empresas</option>{chargeTeams.map(team=><option key={team} value={team}>{team}</option>)}</select><span className="sub">{chargeCandidates.length} O.S. selecionada(s)</span></div>{chargeOpen&&<div style={{marginTop:14,padding:14,border:'1px solid #dce7df',borderRadius:12,background:'#f8fbf9'}}><div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',marginBottom:10}}><div><b>Mensagem de cobrança</b><span className="sub">Revise ou edite antes de copiar.</span></div><button onClick={()=>setChargeOpen(false)}><X size={16}/>Fechar</button></div><textarea value={chargeDraft} onChange={e=>setChargeDraft(e.target.value)} rows={Math.min(18,8+chargeCandidates.length)} style={{width:'100%',resize:'vertical',minHeight:180,padding:12,border:'1px solid #cfdcd3',borderRadius:8,fontFamily:'inherit'}}/><div style={{display:'flex',justifyContent:'flex-end',marginTop:10}}><button className="primary" onClick={copyBulkCharge}><Copy size={16}/>Copiar e registrar cobrança</button></div></div>}</section>
